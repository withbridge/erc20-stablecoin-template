...
contract ERC20TemporaryApprovalUpgradeable is ERC20Upgradeable, IERC7674 {

    using SlotDerivation for bytes32;
    using TransientSlot for bytes32;
    using TransientSlot for TransientSlot.Uint256Slot;

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.ERC20_TEMPORARY_APPROVAL_STORAGE"))
    ...
    /**
     * @dev {allowance} override that includes the temporary allowance when looking up the current
     * allowance. If
     * adding up the persistent and the temporary allowances result in an overflow,
     * type(uint256).max is returned.
     */
    ...
    /**
     * @dev Internal getter for the current temporary allowance that `spender` has over `owner`
     * tokens.
     */
    function _temporaryAllowance(address owner, address spender)
        internal
        view
        virtual
        returns (uint256)
    ...
    /**
     * @dev Alternative to {approve} that sets a `value` amount of tokens as the temporary allowance
     * of `spender` over
     * the caller's tokens.
     *
     ...
     *
     * This internal function is equivalent to `temporaryApprove`, and can be used to e.g. set
    ...
    /**
     * @dev {_spendAllowance} override that consumes the temporary allowance (if any) before
        ...
        override
    {
        ...
        if (currentTemporaryAllowance > 0) {
            // All value is covered by the infinite allowance. nothing left to spend, we can return
            // early
            ...
            unchecked {
                ...
                _temporaryApprove(
                    owner, spender, currentTemporaryAllowance - spendTemporaryAllowance
                );
    ...
    {
        return ERC20_TEMPORARY_APPROVAL_STORAGE.deriveMapping(owner).deriveMapping(spender)
            .asUint256();
    }
